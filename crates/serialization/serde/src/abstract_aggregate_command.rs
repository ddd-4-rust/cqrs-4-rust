//! Jackson-compatible aggregate command using Serde.
...
/// Aggregate command retaining event metadata and concrete aggregate identity.
...
impl<RootId> AbstractAggregateCommand<RootId> {
    /// Creates an aggregate command with all event metadata.
    ...
    /// Creates an aggregate command using current event metadata.
    pub fn new_now(
        event_type: EventType,
        entity_id_path: EntityIdPath,
        aggregate_version: Option<AggregateVersion>,
