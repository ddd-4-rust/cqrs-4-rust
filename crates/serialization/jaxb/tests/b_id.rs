//! JAXB entity ID fixture matching Java `BId`.

use ddd_4_rust_core::{EntityId, EntityType, StringBasedEntityType};
use serde::{Deserialize, Serialize};
use std::sync::OnceLock;

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub(crate) struct BId(String);

impl BId {
    pub(crate) fn new(value: impl Into<String>) -> Self {
        Self(value.into())
    }
}

impl std::fmt::Display for BId {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str(&self.0)
    }
}

impl EntityId for BId {
    fn entity_type(&self) -> &dyn EntityType {
        static ENTITY_TYPE: OnceLock<StringBasedEntityType> = OnceLock::new();
        ENTITY_TYPE.get_or_init(|| StringBasedEntityType::new("B").expect("valid entity type"))
    }

    fn as_string(&self) -> String {
        self.0.clone()
    }
}

#[test]
fn preserves_java_typed_identifier_form() {
    assert_eq!(BId::new("2").as_typed_string(), "B 2");
}
