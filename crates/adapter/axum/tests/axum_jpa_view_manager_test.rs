//! Spring Boot view-manager projection loop checks for Axum.

use async_trait::async_trait;
use chrono::{DateTime, Utc};
use cqrs_4_rust_axum::{AxumJpaViewManager, QryProjectionService};
...
fn build_manager(
    ...
    projection_service: Arc<QryProjectionService>,
) -> AxumJpaViewManager {
...
async fn starts_and_stops_with_no_registered_views() {
    ...
    let manager = AxumJpaViewManager::new(
        Vec::new(),
        store,
        Arc::new(MemoryProjectionAdmin::new()),
        Arc::new(QryProjectionService::new()),
...
async fn tick_applies_events_advances_position_and_is_idempotent() {
    ...
    let projection_service = Arc::new(QryProjectionService::new());
...
async fn start_and_stop_round_trip_with_views() {
    let handled = Arc::new(AtomicU64::new(0));
    let store = Arc::new(MemoryEventStore::new());
    let manager = build_manager(handled, store, Arc::new(QryProjectionService::new()));
