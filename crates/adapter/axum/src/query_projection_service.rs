//! Spring projection service migrated to an Axum-managed service.
//!
//! # Java 对应
//!
//! - 契约：`org.fuin.cqrs4j.esc.ProjectionService`
//! - Spring 适配侧常见实现名：`QryProjectionService`（example / springboot 查询模块）
//!
//! 本类型为进程内实现，用于本地与测试；生产可替换为 SQL/JPA 持久化实现。
...
/// Persists Spring-compatible projection positions for the Axum adapter.
///
/// # Java 对应
///
/// 实现 `org.fuin.cqrs4j.esc.ProjectionService` 的 Spring 查询侧服务
/// （命名对齐 example 中的 `QryProjectionService`）。
#[derive(Debug, Default)]
pub struct QryProjectionService {
    /// In-memory map of stream id → last position（Java 通常落库）.
    positions: RwLock<BTreeMap<String, QryProjectionPosition>>,
}

impl QryProjectionService {
    /// Creates an empty projection service.
    ///
    /// # Java 对应
    ///
    /// Spring 无参 / 注入构造后的空表状态。
    pub fn new() -> Self {
        Self::default()
    }
}
...
#[async_trait]
impl ProjectionService for QryProjectionService {
    ...
    ///
    /// 调用点：`SpringJpaViewManager.handleChunk` /
